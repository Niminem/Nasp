...
proc handleRunCommand(parameters: var Table[string, string]; devMode: bool = true) =
    # allowed flags:
    ...
    var functionData = %*{"function": funcName, "devMode": devMode}
    if args != "":
...
when isMainModule:
    ...
    of run: handleRunCommand(parameters, devMode=true) # run in dev mode by default (todo: make it configurable somehow)
